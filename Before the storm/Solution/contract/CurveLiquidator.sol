...
contract CurveLiquidator {
    ...
    constructor() {
        CRV.approve(CRV_TRICRYPTO_POOL, type(uint256).max);
        CRVUSD.approve(CONTROLLER, type(uint256).max);
        CRVUSD.approve(CRV_TRICRYPTO_POOL, type(uint256).max); // 添加这行来修复授权问题
    }
    ...
    function _withdrawProfits() private {
        ...
        if (crvBalance > 0) {
            CRV.transfer(msg.sender, crvBalance);
        }
    }
}